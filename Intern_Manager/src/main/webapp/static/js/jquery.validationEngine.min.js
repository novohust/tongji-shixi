	...
	var methods = {
		...
		*/
		_onSubmitEvent: function() {
			...
			if(options.onValidationComplete) {
				// !! ensures that an undefined return is interpreted as return false but allows a onValidationComplete() to possibly return true and have form continue processing
				//onValidationComplete在手动调用validate方法对一个表单或一组input验证之后，或
				//提交表单之前自动对整个form validate之后这两种情况下会被触发，增加一个参数表明是否在提交之前被调用的
				return !!options.onValidationComplete(form, r,true);
			}
			return r;
		},
		/**
		...
		_calculatePosition: function (field, promptElmt, options) {
			...
			switch (positionType) {
				...
				case "centerRight":
					promptTopPosition = fieldTop;
					marginTopSize = 0;
					promptleftPosition= fieldLeft + field.outerWidth(true)+5+4;
					break;
				case "centerLeft":
